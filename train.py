...
parser.add_argument('-epoch_start', help='Starting epoch number of the training', default=28, type=int) ##################################
...
# --- Load training data and validation/test data --- #
lbl_train_data_loader = DataLoader(TrainData(crop_size, train_data_dir,labeled_name), batch_size=train_batch_size, shuffle=True, num_workers=8, pin_memory = True)######################
...
#-------------------------------------------------------------------------------------------------------------
    ...
    torch.save(net.state_dict(), './{}/latest.pth'.format(exp_name))#############################################
    ...
    if val_psnr1 >= old_val_psnr1:
        torch.save(net.state_dict(), './{}/best.pth'.format(exp_name))#########################################
        print('model saved')
