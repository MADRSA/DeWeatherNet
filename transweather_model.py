...
class EncoderTransformer(nn.Module):
    ...
    def forward_features(self, x):
        ...
        x1 = x1.reshape(B, H1, W1, -1).permute(0, 3, 1, 2).contiguous()
        outs.append(x1)
        # print("stage 3 x1",x1.shape) #######################################################################################################333

        for i, blk in enumerate(self.patch_block3):
            x2 = blk(x2, H2, W2)
        x2 = self.pnorm3(x2)
        x2 = x2.reshape(B, H2, W2, -1).permute(0, 3, 1, 2).contiguous()
        # print("stage 3 x2",x2.shape) #######################################################################################################333
...
class OverlapPatchEmbed(nn.Module):  # return  patch embeddings, original height and  original width
    """ Image to Patch Embedding
    """

    def __init__(self, img_size=224, patch_size=7, stride=4, in_chans=3, embed_dim=768):
        super().__init__()
        img_size = to_2tuple(img_size)   # convert into tuple : output(224,224)
    ...
    def forward(self, x):  # input = (Batch, channels, height, width) = (B, 3, 256, 256)
        # pdb.set_trace()
        x = self.proj(x)         # x = (Batch, 768, 64, 64)
        _, _, H, W = x.shape
        x = x.flatten(2).transpose(1, 2) # x= (Batch, 4096, 768)
        x = self.norm(x)

        return x, H, W   # (Batch, 4096, 768) , 64, 64
...
class Block_dec(nn.Module):
    ...
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,   # dim = embed dimension
...
class Block(nn.Module):
    ...
    def forward(self, x, H, W):   # input = x(Batch, 4096, 768) whatever is the output of OverPatchembed 
...
class DecoderTransformer(nn.Module):
    ...
    def forward_features(self, x):   # deocder is only using last x1 output from encoder 
        x=x[3]
        # print("Decoder input shape taking outs[3] from encoder output", x.shape) ##########################################################
        B = x.shape[0]
        outs = []
...
class Tenc(EncoderTransformer):
    def __init__(self, **kwargs):
        super(Tenc, self).__init__(
            patch_size=4, embed_dims=[64, 128, 320, 512], num_heads=[1, 2, 4, 4], mlp_ratios=[2, 2, 2, 2],
            qkv_bias=True, norm_layer=partial(nn.LayerNorm, eps=1e-6), depths=[2, 2, 2, 2], sr_ratios=[4, 2, 2, 1],  
            drop_rate=0.0, drop_path_rate=0.1) ############################     depths=[3, 3, 2, 2] in chnaged one, actual [2,2,2,2]
